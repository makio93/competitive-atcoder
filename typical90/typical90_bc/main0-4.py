        ...
        if len(mulsr[ri-1]) > 0:
            if ldi != 0:
                ...
                if g == 1:
                    ...
                    if rdi in mulsr[ri-1].keys():
                        res += mulsl[li-1][ldi] * mulsr[ri-1][rdi]
                elif q == 0:
                    if 0 in mulsr[ri-1].keys():
                        res += mulsl[li-1][ldi] * mulsr[ri-1][0]
                    if p % g == 0:
                        rdi = p // g
                        for rdik in range(rdi, p, rdi):
                            if rdik in mulsr[ri-1].keys():
                                res += mulsl[li-1][ldi] * mulsr[ri-1][rdik]
                elif q % g == 0:
