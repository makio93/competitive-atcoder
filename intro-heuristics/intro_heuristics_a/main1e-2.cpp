...
// 一点変更+二点スワップ、加えてスコア計算を高速化、ちょっとだけ変化
...
public:
	...
	bool change_state(input& in, bool mode, int p, int q) {
		if (mode) {
			if (stat[p] == q) return false;
			ll old_diff = diff_score(in, stat[p], q);
			int pc = stat[p];
			ds[pc].erase(p+1);
			stat[p] = q;
			ds[q].insert(p+1);
			...
			if (p==q || stat[p]==stat[q]) return false;
...
void rsolve(input& in, output& out, double tl=1.9) {
    ...
    while (now_time() < tl) {
		if (rand()/(double)(RAND_MAX) < 0.5) {
			...
			if (st.stat[nd] == nc) continue;
			//while (st.stat[nd] == nc) { nd = rand() % in.d; nc = rand() % 26; }
			st.change_state(in, true, nd, nc);
		}
		else {
			int dl = rand() % in.d, dlen = min(in.d, dl+16) - dl, dr = dl + rand() % dlen;
			if (dl==dr || st.stat[dl]==st.stat[dr]) continue;
			/*
			while (dl==dr || st.stat[dl]==st.stat[dr]) {
				dl = rand() % in.d; dlen = min(in.d, dl+16) - dl; dr = dl + rand() % dlen;
			}
			*/
