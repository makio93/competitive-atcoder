...
void display_time(clock_t nt=0L) { printf("changed : %.10f\n", now_time(nt)); }
...
public:
	...
	bool change_state(input& in, bool mode, int p, int q) {
		if (mode) {
			...
			swap(stat[p], stat[q]);
			ll next_diff = diff_score(in, stat[p], stat[q]);
			if (next_diff > old_diff) {
				score = next_diff - old_diff;
				return true;
			}
...
void rsolve(input& in, output& out, double tl=9.9) {
	srand(time(NULL));
    ...
    while (now_time() < tl) {
		if (rand()/(double)(RAND_MAX) < 0.5) {
			...
			if (st.change_state(in, true, nd, nc)) display_time();
		}
		else {
			int dl = rand() % in.d, dlen = min(in.d, dl+16) - dl, dr = dl + rand() % dlen;
			if (st.change_state(in, false, dl, dr)) display_time();
		}
    }
	out = st.output_states();
}
