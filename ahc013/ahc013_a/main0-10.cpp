...
bool pmove(int ri, int ci, bool hor, vector<string>& c, map<int, int>& vids, vector<vector<bool>>& used, 
	vector<tuple<int, int, int, int>>& rlst, vector<tuple<int, int, int, int>>& tmlst, int mcnt) {
	if ((int)(rlst.size()+tmlst.size())+mcnt+2 > k*100) return false;
	if (hor) {
		if (ri-1>=0 && c[ri-1][ci]=='0' && !used[ri-1][ci] && (ri+1>=n||(ri+1<n&&!used[ri+1][ci]))) {
		...
		else if (ri+1<n && c[ri+1][ci]=='0' && !used[ri+1][ci] && (ri-1<0||(ri-1>=0&&!used[ri-1][ci]))) {
			int pid = vids[ri*n+ci];
			vids.erase(ri*n+ci);
			vids[(ri+1)*n+ci] = pid;
			c[ri+1][ci] = c[ri][ci];
		...
		if (ci-1>=0 && c[ri][ci-1]=='0' && !used[ri][ci-1] && (ci+1>=n||(ci+1<n&&!used[ri][ci+1]))) {
			int pid = vids[ri*n+ci];
			vids.erase(ri*n+ci);
			vids[ri*n+ci-1] = pid;
			c[ri][ci-1] = c[ri][ci];
		...
		else if (ci+1<n && c[ri][ci+1]=='0' && !used[ri][ci+1] && (ci-1<0||(ci-1>=0&&!used[ri][ci-1]))) {
			int pid = vids[ri*n+ci];
			vids.erase(ri*n+ci);
			vids[ri*n+ci+1] = pid;
			c[ri][ci+1] = c[ri][ci];
...
void rsearch(int k1, vector<string>& c, map<int, int>& vids, vector<vector<bool>>& used, dsu& glst, vector<vector<int>>& kcnt, 
	vector<tuple<int, int, int, int>>& rlst, bool& ful, vector<tuple<int, int, int, int>>& tmlst, int mcnt=0) {
	if (ful) return;
	rep(i, n) {
		int li = -1, pli = -1;
		rep(j, n) {
					...
					if (!glst.same(vids[i*n+li], vids[i*n+j])) {
						if (pli != -1) {
							if ((int)(glst.size(vids[i*n+li]))==1 && pmove(i, li, true, c, vids, used, rlst, tmlst, mcnt)) {
								...
								rlst.emplace_back(i, pli, i, j);
								rep(ii, k) kcnt[glst.leader(vids[i*n+j])][ii] = nkcnt[ii];
								rep3(ii, pli+1, j) used[i][ii] = true;
								li = pli;
...
void csearch(int k1, vector<string>& c, map<int, int>& vids, vector<vector<bool>>& used, dsu& glst, vector<vector<int>>& kcnt, 
	vector<tuple<int, int, int, int>>& rlst, bool& ful, vector<tuple<int, int, int, int>>& tmlst, int mcnt=0) {
	if (ful) return;
	rep(i, n) {
		int ui = -1, pui = -1;
					...
					if (!glst.same(vids[ui*n+i], vids[j*n+i])) {
						if (pui != -1) {
							if ((int)(glst.size(vids[ui*n+i]))==1 && pmove(ui, i, false, c, vids, used, rlst, tmlst, mcnt)) {
								...
								rlst.emplace_back(pui, i, j, i);
								rep(ii, k) kcnt[glst.leader(vids[j*n+i])][ii] = nkcnt[ii];
								rep3(ii, pui+1, j) used[ii][i] = true;
								ui = pui;
								if ((int)(rlst.size()+tmlst.size())+mcnt >= k*100) {
									ful = true;
									return;
...
int main() {
	...
	vector<int> ord(k);
	iota(all(ord), 0);
	tuple<int, tuple<vector<tuple<int, int, int, int>>, vector<tuple<int, int, int, int>>, 
		vector<string>, map<int, int>>, pair<vector<int>, int>> res =
		{ -INF, { vector<tuple<int, int, int, int>>(), vector<tuple<int, int, int, int>>(), c, vids }, { vector<int>(), -1 } };
	do {
		rep(bi, 2) {
			vector<vector<bool>> used(n, vector<bool>(n));
			vector<tuple<int, int, int, int>> rlst, mlst;
			bool ful = false;
			dsu glst(k*100);
	...
	vector<tuple<int, int, int, int>> rlst = get<0>(get<1>(res)), mlst = get<1>(get<1>(res));
	c = get<2>(get<1>(res));
