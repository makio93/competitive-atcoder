...
int main() {
	...
	int sco = res.first;
	vector<tuple<int, int, int, int>> rlst = res.second;
	vector<int> vtoc(k*100);
	for (auto& co : vids) vtoc[co.second] = co.first;
	random_device seed_gen;
	default_random_engine engine(seed_gen());
	uniform_int_distribution<> dcnt(1, 5), vchoose(0, k*100-1), dchoose(0, 3);
	int mcnt = 0;
	vector<tuple<int, int, int, int>> mlst;
	const vector<int> dy = { -1, 0, 1, 0 }, dx = { 0, 1, 0, -1 };
	while ((double)((clock()-start)/CLOCKS_PER_SEC) <= 2.7) {
		auto nc = c;
		auto nvids = vids;
		auto nvtoc = vtoc;
		int di = dcnt(engine) * 30;
		mcnt += di;
		vector<tuple<int, int, int, int>> amlst;
		rep(i1, di) {
			while (1) {
				int vc = vchoose(engine), dc = dchoose(engine);
		...
		vector<int> mord(k);
		iota(all(mord), 0);
		pair<int, vector<tuple<int, int, int, int>>> nres = { -1, vector<tuple<int, int, int, int>>() };
		do {
			rep(mbi, 1<<k) {
				vector<vector<bool>> used(n, vector<bool>(n));
				...
				nres = max(nres, { nsco, nrlst });
			}
		} while (next_permutation(all(mord)));
		if (nres.first > sco) {
			sco = nres.first;
			rlst = nres.second;
			c = nc;
