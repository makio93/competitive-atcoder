...
int main() {
			...
			auto rsearch = [&](int i1) -> void {
				...
				rep(i, n) {
					...
					rep(j, n) {
						...
						else if (li!=-1 && c[i][j]==(char)('1'+ord[pi1]) && (int)(rlst.size())%5<k-1) {
							pli = li;
							li = j;
						}
						else li = pli = -1;
			...
			auto csearch = [&](int i1) -> void {
				...
				rep(i, n) {
					...
					rep(j, n) {
						...
						else if (ui!=-1 && c[j][i]==(char)('1'+ord[pi1]) && (int)(rlst.size())%5<k-1) {
							pui = ui;
							ui = j;
						}
						else ui = pui = -1;
	...
	uniform_int_distribution<> dcnt(1, (1<<k)), vchoose(0, 99), dchoose(0, 3), cdi(1, k*3/2);
	int mcnt = 0;
		...
		auto rsearch = [&](int i1) -> void {
			...
			rep(i, n) {
				...
				rep(j, n) {
					...
					else if (li!=-1 && nc[i][j]==(char)('1'+mord[pi1]) && ((int)(nrlst.size())+mcnt)%5<k-1) {
						pli = li;
						li = j;
					}
					else li = pli = -1;
		...
		auto csearch = [&](int i1) -> void {
			...
			rep(i, n) {
				...
				rep(j, n) {
					...
					else if (ui!=-1 && nc[j][i]==(char)('1'+mord[pi1]) && ((int)(nrlst.size())+mcnt)%5<k-1) {
						pui = ui;
						ui = j;
					}
					else ui = pui = -1;
