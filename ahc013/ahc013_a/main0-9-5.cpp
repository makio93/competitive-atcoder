...
int main() {
	...
	do {
		rep(bi, 1<<k) {
			...
			rep(i1, k) {
				if ((bi>>i1)&1) {
					rsearch(ord[i1], c, vids, used, glst, kcnt, rlst, ful);
					csearch(ord[i1], c, vids, used, glst, kcnt, rlst, ful);
				}
				else {
					csearch(ord[i1], c, vids, used, glst, kcnt, rlst, ful);
					rsearch(ord[i1], c, vids, used, glst, kcnt, rlst, ful);
	...
	do {
		...
		rep(i1, k) {
			if ((mbi>>i1)&1) {
				rsearch(mord[i1], nc, nvids, used, glst, kcnt, nrlst, ful, mcnt);
				csearch(mord[i1], nc, nvids, used, glst, kcnt, nrlst, ful, mcnt);
			}
			else {
				csearch(mord[i1], nc, nvids, used, glst, kcnt, nrlst, ful, mcnt);
				rsearch(mord[i1], nc, nvids, used, glst, kcnt, nrlst, ful, mcnt);
