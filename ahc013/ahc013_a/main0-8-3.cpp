...
int main() {
	...
	do {
		rep(bi, 2) {
			...
			bool ful = false;
			dsu glst(k*100);
			auto rsearch = [&](int i1, int co=2) -> void {
				if (ful) return;
				...
				rep(i, n) {
					...
					rep(j, n) {
						...
						else if (li!=-1 && c[i][j]==(char)('1'+ord[pi1]) && (int)(rlst.size())%5<k-co) {
							pli = li;
							li = j;
						}
						else li = pli = -1;
			...
			auto csearch = [&](int i1, int co=2) -> void {
				if (ful) return;
				int pi1 = min(k-1, i1+(i1%2==0?1:0));
				if (pi1 == i1) return;
				rep(i, n) {
					...
					rep(j, n) {
						...
						else if (ui!=-1 && c[j][i]==(char)('1'+ord[pi1]) && (int)(rlst.size())%5<k-co) {
							pui = ui;
							ui = j;
						}
						else ui = pui = -1;
			...
			if (bi == 0) {
				rep(i1, k) rsearch(i1);
				rep(i1, k) csearch(i1);
				rep(i1, k) rsearch(i1, 7);
				rep(i1, k) csearch(i1, 7);
			}
			else {
				rep(i1, k) csearch(i1);
				rep(i1, k) rsearch(i1);
		...
		auto rsearch = [&](int i1, int co=2) -> void {
			...
			rep(i, n) {
				...
				rep(j, n) {
					...
					else if (li!=-1 && nc[i][j]==(char)('1'+mord[pi1]) && ((int)(nrlst.size())+mcnt)%5<k-co) {
						pli = li;
						li = j;
					}
					else li = pli = -1;
		...
		auto csearch = [&](int i1, int co=2) -> void {
			...
			rep(i, n) {
				...
				rep(j, n) {
					...
					else if (ui!=-1 && nc[j][i]==(char)('1'+mord[pi1]) && ((int)(nrlst.size())+mcnt)%5<k-co) {
						pui = ui;
						ui = j;
					}
					else ui = pui = -1;
