...
bool check3(int r1, int c1, int r2, int c2, int r3, int c3, 
	...
	return val2 > (int)(val1 * (9.0 + (clock()-start)/(double)CLOCKS_PER_SEC) / 10 + 0.5);
}
...
int main() {
	...
	do {
		rep(bi, 2) {
			...
			if (bi == 0) {
				rep(i1, k) rsearch(ord[i1], c, vids, used, glst, kcnt, rlst, ful);
				rep(i1, k) csearch(ord[i1], c, vids, used, glst, kcnt, rlst, ful);
			}
			else {
				rep(i1, k) csearch(ord[i1], c, vids, used, glst, kcnt, rlst, ful);
				rep(i1, k) rsearch(ord[i1], c, vids, used, glst, kcnt, rlst, ful);
	...
	do {
		...
		rep(ii, k) rep(jj, 100) kcnt[ii*100+jj][ii] = 1;
		if (mbi == 0) {
			rep(i1, k) rsearch(mord[i1], nc, nvids, used, glst, kcnt, nrlst, ful, mcnt);
			rep(i1, k) csearch(mord[i1], nc, nvids, used, glst, kcnt, nrlst, ful, mcnt);
		}
		else {
			rep(i1, k) csearch(mord[i1], nc, nvids, used, glst, kcnt, nrlst, ful, mcnt);
			rep(i1, k) rsearch(mord[i1], nc, nvids, used, glst, kcnt, nrlst, ful, mcnt);
