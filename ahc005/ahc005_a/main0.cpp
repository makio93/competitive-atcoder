...
int main() {
	...
	vector<map<set<int>, int>> dist(n*n);
	...
	while (!que.empty()) {
		...
		rep(i2, 4) {
			...
			for (int vi : st) nst.insert(vi);
			if (dist[ny*n+nx].find(nst)!=dist[ny*n+nx].end() && dist[ny*n+nx][nst]<=nd) continue;
