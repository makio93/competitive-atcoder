...
clock_t start = clock();
bool timecheck(double tl) { return (double)(clock()-start)/CLOCKS_PER_SEC <= tl; }
bool timecheck(clock_t sst, double stl) { return (double)(clock()-sst)/CLOCKS_PER_SEC <= stl; }
...
struct TimeUnionFind {
	...
	TimeUnionFind *prev;
	TimeUnionFind(int n_, dsu *ds_) {
		n = n_;
		ds = ds_;
		step = 0, this->prev = NULL, pa = -1, pb = -1;
		rep(i, n) toleader[i] = ds->leader(i);
	}
	TimeUnionFind(TimeUnionFind *pds, int a, int b) {
		n = pds->n;
		prev = pds;
		ds = new dsu(n);
		toleader = prev->toleader;
		auto glst = prev->ds->groups();
		int glen = glst.size(), ai = -1, bi = -1;
		rep(i, glen) {
			int vlen = glst[i].size();
			rep(j, vlen-1) ds->merge(glst[i][j], glst[i][j+1]);
			if (ds->same(a, glst[i][0])) ai = i;
			if (ds->same(b, glst[i][0])) bi = i;
		}
		int ci = leader(ds->merge(a, b)), alen = glst[ai].size(), blen = glst[bi].size();
		rep(i, alen) toleader[glst[ai][i]] = ci;
		rep(i, blen) toleader[glst[bi][i]] = ci;
		step = prev->step + 1;
		pa = a, pb = b;
	}
	~TimeUnionFind() { if (ds != NULL) delete ds; }
	int leader(int a) { return toleader.find(a) != toleader.end() ? toleader[a] : (toleader[a] = ds->leader(a)); }
};
...
struct TUFManager {
	...
	TUFManager(int n, dsu *ds_, bool is_back_=false) {
		is_back = is_back_;
		dsu *tds = ds_;
		ds = new dsu(n);
		auto glst = tds->groups();
		int glen = glst.size();
		rep(i, glen) {
			int vlen = glst[i].size();
			rep(j, vlen-1) ds->merge(glst[i][j], glst[i][j+1]);
	...
	int merge(int a, int b) {
		if (is_back) {
			if (now->ds->same(a, b)) return now->leader(a);
			now = new TimeUnionFind(now, a, b);
			db.emplace(now->step, now);
			return now->leader(a);
		}
		else return ds->merge(a, b);
	}
	bool same(int a, int b) { return is_back ? now->ds->same(a, b) : ds->same(a, b); }
	int leader(int a) { return is_back ? now->leader(a) : ds->leader(a); }
	int size(int a) { return is_back ? now->ds->size(a) : ds->size(a); }
	bool undo() {
...
struct Cube {
	...
	void init_data() {
		...
		auto dfs = [&](auto dfs, set<tuple<int, int, int>>& pst, int xi, int yi, int zi) -> void {
			...
			rep(i, 6) {
				...
				tds->merge(cell_id(xi, yi, zi), cell_id(nx, ny, nz));
		...
		rep(i, d) rep(j, d) rep(k, d) if (c[i][j][k] && !visited[i][j][k]) {
			...
			blst[tds->leader(cell_id(i, j, k))] = slst.find(str);
			pst.clear();
		}
		norm_wall(tds);
		...
		blocks = new TUFManager(d*d*d, tds, d<=9);
	}
	void init_cube() {
	...
	void norm_wall(dsu *tds) {
		rep(i, d) rep(j, d) rep(k, d) if (c[i][j][k]) {
			if (i+1<d && c[i+1][j][k] && tds->same(cell_id(i,j,k),cell_id(i+1,j,k)) && xw[i][j][k].second) {
				xw[i][j][k].second = xw[i+1][j][k].first = false;
			}
			if (j+1<d && c[i][j+1][k] && tds->same(cell_id(i,j,k),cell_id(i,j+1,k)) && yw[i][j][k].second) {
				yw[i][j][k].second = yw[i][j+1][k].first = false;
			}
			if (k+1<d && c[i][j][k+1] && tds->same(cell_id(i,j,k),cell_id(i,j,k+1)) && zw[i][j][k].second) {
...
public:
	...
	pair<int, int> choose_walls() {
		...
		vector<bool> used(wlen);
		while (mval.second.first==INF && wlen>0) {
			int rid = (*engine)() % wlen, ti = 0;
			while (ti < (int)(walls_list.size())) {
				if (!used[ti]) {
					--rid;
					if (rid < 0) break;
				}
			...
			if (!timecheck(5.2) || !timecheck(sstart, stl)) break;
		}
		if (mval.second.first == INF) mval.second.first = -1;
		return mval.second;
	}
	double eval_cubes() {
		double sval = 0.0, stl = 5.0;
		clock_t sstart = clock();
		int scnt = 0;
		vector<int> walls_list(all(c1->target_walls));
		int wlen = walls_list.size();
		...
		while (wlen > 0) {
			...
			int rval = c2->search_wall(s1, s2, s3);
			if (rval != -1) {
				double tdiff = calc_diff(s1, s2, s3);
				if (tdiff > 0.0) {
					sval = max(sval, tdiff);
					++scnt;
				}
			}
			if (!timecheck(5.0) || !timecheck(sstart, stl)) break;
		}
		if (scnt == 0) sval = -DINF;
		return sval;
	}
	...
	pair<int, int> search_choice() {
		double stl = 5.0;
		clock_t sstart = clock();
		pair<pair<double, double>, pair<int, int>> mval = { { 0.0, 0.0 }, { INF, -1 } };
		for (const int& ti : c1->target_walls) {
			...
			if (rval != -1) {
				...
				if (pre > 0.0) {
					remove_walls(nwids);
					mval = max(mval, { { pre+eval_cubes(), pre }, nwids });
					repair_walls(nwids);
				}
			}
			if (!timecheck(5.0) || !timecheck(sstart, stl)) break;
	...
	bool proceed() {
		auto pi = (d <= 9) ? search_choice() : choose_walls();
		if (pi.first == -1) return false;
		remove_walls(pi);
		return true;
	}
...
int main() {
	auto pzl = input();
	while (timecheck(5.0)) pzl.proceed();
	output(pzl);
	return 0;
}
