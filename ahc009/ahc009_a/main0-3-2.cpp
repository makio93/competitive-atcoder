...
int main() {
	...
	while (!que.empty()) {
		...
		if (ndr != -1) {
			...
			dist[ni][nj][ndr][disti+1] += pi * (1.0 - p) / 4.0;
			que.emplace(ni, nj, ndr, disti+1, dist[ni][nj][ndr][disti+1]);
			dist[vi][vj][4][disti+1] += pi * p;
	...
	int stai = (int)(get<2>(mval) * 1.0 / (1.0-p)), posi = ti, posj = tj;
	string nres;
	srand(time(NULL));
	rep3r(i, 1, stai+1) {
		...
		if (mval1.second != 4) {
			...
			posi += rdi[mval1.second], posj += rdj[mval1.second];
		}
		else if (rand()/(double)RAND_MAX >= p/5.0) {
			nres.push_back(dir[mval2.second]);
		}
		if (posi==si && posj==sj) mres = max(mres, { mval1.first*(stai-i+1), nres });
	}
