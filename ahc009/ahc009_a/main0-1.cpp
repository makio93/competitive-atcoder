...
int main() {
	...
	dist[si][sj][4][0] = 1.0;
	auto next_dir = [&](int nowi, int nowj) -> int {
		const int INF = (int)(1e9);
	...
	while (!que.empty()) {
		...
		rep(i1, 4) {
			int ni = vi + di[i1], nj = vj + dj[i1];
			if (ni<0 || ni>=20 || nj<0 || nj>=20 || wall[vi*20+vj][ni*20+nj]) continue;
			dist[ni][nj][i1][disti+1] += pi * p;
			que.emplace(ni, nj, i1, disti+1, dist[ni][nj][i1][disti+1]);
		}
	...
	rep3r(i, 1, stai+1) {
		...
		res.push_back(dir[mval2.second]);
