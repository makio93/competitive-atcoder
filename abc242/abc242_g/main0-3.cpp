...
public:
    ...
    int find(int l, int r) {
        ...
        if (L < R) {
            queue<unordered_map<int, int>> mlst;
            for (int i=L; i<R; ++i) mlst.push(bucket[i]);
            while ((int)(mlst.size()) > 1) {
                auto lst1 = mlst.front(); mlst.pop();
                auto lst2 = mlst.front(); mlst.pop();
                for (auto pi : lst2) lst1[pi.first] += pi.second;
                mlst.push(lst1);
            }
            scnt = mlst.front();
