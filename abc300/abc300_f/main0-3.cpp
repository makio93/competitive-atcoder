...
int main() {
	...
	if (m == 1) {
		...
		rep(i, xlen) {
			...
			res = max(res, min((ll)m*n, (ll)(xids[ri]-xids[i]-1)));
		}
		cout << res << endl;
		return 0;
	}
	...
	rep(i, xlen) {
		...
		res = max(res, min((ll)m*n-xids[i]-1, (xids[ri]-xids[i]-1)+aval*n));
	}
	res = max(res, min((ll)m*n, max(n*2-1-xids[xlen-1-bval]+aval*n, xids[bval]+aval*n)));
	cout << res << endl;
	return 0;
}
