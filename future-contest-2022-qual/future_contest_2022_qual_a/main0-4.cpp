...
int main() {
	...
	uniform_int_distribution<> randint(0, 5*k-1);
	...
	rep(i, 2000) {
		...
		while (!workers.empty() && !tasks.empty()) {
			...
			int sid = smval.second;
			res.emplace_back(sid+1, tid+1);
			int tsum = 0;
			rep(j, k) tsum += max(0, d[tid][j]-s[sid][j]);
			working[sid] = tid;
			sches[sid] = i;
			workers.erase(sid);
			tasks.pop();
		...
		rep(j, nlen) {
			...
			if ((int)(slogs[fi].size())%10 == 3) {
				rep(j1, 80*(int)(slogs[fi].size())) {
					clock_t now_t = clock();
					if ((now_t-start_t)/(double)(CLOCKS_PER_SEC) > 1.8*i/2000) break;
					int nsum = 0;
					vector<int> ns = mlst.second;
					rep(j2, 3) {
						int rval = randint(engine), add = rval/k - 2, tki = rval % k;
						ns[tki] += add;
					}
					for (auto pi : slogs[fi]) rep(j2, k) nsum += max(0, d[pi.first][j2]-ns[j2]);
