...
int main() {
	...
	map<int, pair<int, deque<pair<int, int>>>> nsches;
	...
	rep(i, 2000) {
		...
		workers.insert(-1);
		for (auto sitr=workers.begin(); sitr!=workers.end(); ++sitr) if (*sitr!=-1 && nsches.find(*sitr)!=nsches.end()) {
			int sid = *sitr, tid = nsches[sid].second.front().first, ptcost = nsches[sid].second.front().second;
			res.emplace_back(sid+1, tid+1);
			sitr = workers.erase(sitr);
			--sitr;
			working[sid] = tid;
			sches[sid] = i;
			int tcost = 0;
			rep(j2, k) tcost += max(0, d[tid][j2]-s[sid][j2]);
			tcost = max(1, tcost);
			fsches[sid] = i + tcost;
			nsches[sid].first -= ptcost;
			nsches[sid].second.pop_front();
			if (nsches[sid].second.empty()) nsches.erase(sid);
		}
		workers.erase(-1);
		if (!workers.empty()) {
			...
			rep(j1, m) rep(j2, ttlen) {
				...
				if (working.find(j1) != working.end()) {
					sadd += fsches[j1] - i;
					if (nsches.find(j1) != nsches.end()) sadd += nsches[j1].first;
				}
				rep(ji, k) tcost += max(0, d[ttid][ji]-s[j1][ji]);
				tcost = max(1, tcost) + sadd;
				tg.add_edge(j1, m+j2, 1, tcost);
			}
			rep(j, m) tg.add_edge(m+ttlen, j, max(1,(int)(n/m*(double)i/2000*i/2000+0.5)), 0);
			rep(j, ttlen) tg.add_edge(m+j, m+ttlen+1, 1, 0);
			tg.flow(m+ttlen, m+ttlen+1, ttlen);
			unordered_set<int> ustasks;
			vector<set<pair<pair<int, int>, int>>> sdlst(m);
			rep(j, m*ttlen) if (tg.get_edge(j).flow > 0) {
				int sid = tg.get_edge(j).from, did = contasks[tg.get_edge(j).to-m].first, tid = contasks[tg.get_edge(j).to-m].second;
				int tcost = 0;
				rep(j2, k) tcost += max(0, d[tid][j2]-s[sid][j2]);
				tcost = max(1, tcost);
				sdlst[sid].emplace(make_pair(did, tcost), tid);
			}
			rep(j, m) {
				for (auto sitr=sdlst[j].begin(); sitr!=sdlst[j].end(); ++sitr) {
					int sid = j, tid = sitr->second, tcost = sitr->first.second;
					if (workers.find(sid) != workers.end()) {
						res.emplace_back(sid+1, tid+1);
						workers.erase(sid);
						working[sid] = tid;
						sches[sid] = i;
					...
					else if (nsches.find(sid)==nsches.end() || (int)(nsches[sid].second.size())<=max(1,(int)(n/m*(double)i/2000*i/2000+0.5))) {
						nsches[sid].first += tcost;
						nsches[sid].second.emplace_back(tid, tcost);
						ustasks.insert(tid);
					}
				}
			}
