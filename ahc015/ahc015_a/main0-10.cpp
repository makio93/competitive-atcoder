...
pair<int, int> next_direction(vector<vector<int>>& box, mt19937_64& engine, vector<int>& f, int id, int ki, int ti=0) {
	pair<int, int> rval = { -INF, -1 };
	rep(rid, 4) {
		auto nbox = box;
		lean(nbox, rid);
		if (id+1>=100 || ti==ki) rval = max(rval, { score_calc(nbox), rid });
		else {
			ll sum = 0, pval = score_calc(nbox);
			int len = 100 - id - 1, slen = len;
			rep(i, 1) len = (len+(rate+1)/2) / rate;
			vector<int> ids(len);
			rep(i, len) ids[i] = engine() % slen;
			int tid = 0, tcnt = 0;
			rep(i, 10) rep(j, 10) if (nbox[i][j] == 0) {
				if (tid<len && ids[tid]==tcnt) {
					nbox[i][j] = f[id+1];
					auto tval = next_direction(nbox, engine, f, id+1, ki, ti+1);
					sum += tval.first;
					nbox[i][j] = 0;
					++tid;
				}
...
int main() {
	...
	rep(i, n) {
		...
		mval = max(mval, next_direction(box, engine, f, i, 3));
		int rid = mval.second;
		lean(box, rid);
		cout << rc[rid] << endl;
		cout.flush();
