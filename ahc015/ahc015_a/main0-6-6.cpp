...
int operate_test_all(vector<vector<int>>& box, int fid, int yi, int xi, int sid=0, int mid=1) {
	auto lbox = box, ubox = box;
	int li = 0, ui = 0;
	rep3(i, xi+1, 10) if (lbox[yi][i] == fid) ++li;
	rep3(i, yi+1, 10) if (ubox[i][xi] == fid) ++ui;
	lean(lbox, 1);
	lean(ubox, 2);
	if (score_calc(lbox, sid, mid) >= score_calc(ubox, sid, mid)) return 1;
	else return 2;
}
int dir_choose(vector<vector<int>>& box, int fid, int yi, int xi, int fi) {
	...
	if (fi == 0) {
		...
		else return operate_test(nbox, fid, yi, xi, 8);
	}
	else if (fi >= 1) {
		if (pval.first>0 && pval.second>0) {
			if (pval.first > pval.second) return 3;
		...
		else if (pval.first>0 || pval.second>0) return operate_test(nbox, fid, yi, xi, fi*2);
		else return operate_test_all(nbox, fid, yi, fi*2);
	}
}
